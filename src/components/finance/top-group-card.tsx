  ...
  return (
    <Card className="overflow-hidden shadow-sm">
      ...
      <div className="w-full flex items-center justify-between p-3 hover:bg-muted/50 transition-colors">
        <button
          onClick={() => setOpen((o) => !o)}
          className="flex items-center gap-2 flex-1 touch-manipulation"
        >
          <ChevronDown
            className={cn(
          ...
          />
          <span className="font-semibold text-sm">{topLabel}</span>
          <span className="text-xs text-muted-foreground">({topCats.length})</span>
        </button>
    ...
    <Popover open={open} onOpenChange={(o) => {
      ...
      <PopoverTrigger asChild>
        <button
          ...
          className={cn(
            'p-1 rounded-md hover:bg-muted text-muted-foreground hover:text-foreground transition-colors touch-manipulation',
            'opacity-0 group-hover:opacity-100'
          )}
          aria-label="Renomear"
        >
          <Pencil className={small ? 'h-2.5 w-2.5' : 'h-3 w-3'} />
        </button>
      </PopoverTrigger>
